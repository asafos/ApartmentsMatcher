...
func main() {
	...
	if err != nil {
		...
		if db == nil {
			...
			err = app.DB.AutoMigrate(&models.Apartment{})
			if err != nil {
				fmt.Println("failed to automigrate apartment model:", err.Error())
				return
			}
			err = app.DB.AutoMigrate(&models.ApartmentPref{})
			if err != nil {
				fmt.Println("failed to automigrate apartmentPref model:", err.Error())
				return
			}
		}
	}
